...
الرجاء مراقبة الصورة بدقة شديدة واستخراج الآتي حصراً:
1. الحروف المعروضة كخيارات متناثرة أسفل الشاشة (لوحة المفاتيح المتاحة).
2. وصف معمق ودقيق لمحتوى الصورة الاستدلالية الرئيسية في الأعلى.
3. تحديد نوع المرحلة الظاهرة بوضوح (كلمات مبعثرة، كلمات متقاطعة، أمثال، مقاطع).
4. في الكلمات المبعثرة: استخرج عدد المربعات (أطوال الكلمات) مثل [4, 5]. 
   في الكلمات المتقاطعة: اقرأ الكلمات الموجودة في الشبكة، ومثّل المربع الفارغ المخصص للإجابة (المربع الأزرق أو المضيء) بعلامة `_` (مثال: `م _ س و م`).
5. في حالة توفر إيموجيات فقط (لمرحلة الأمثال)، قم باستخراجها بدقة.
...
{{
  ...
  "image_description": "وصف دقيق لما يظهر في الصورة (مثال: ساحر يقطع امرأة في صندوق بمنشار)",
  "available_letters": "ق، ي، ن، و... (الحروف في الأسفل)",
  "word_lengths": [4, 5],
  "partial_words": ["م _ س و م", "م ف ت ا ح"],
